...
"""
TCP_cli = TCP_Client('8.130.14.55',8848)
UDP_cli = UDP_Client('8.130.14.55',8848)
   ...
   """
    while True:
        command = input("Please select the command to be performed(input the corresponding serial number)\n "\
                        ...
                        "3.Onchain(push the messages on the server to the Ethereum) \n"\
                        "4.Authority_Grant(grant the authority to other ROS-Chain users) \n"\
                        "5.Info_Check(Check the messages on the Ethereum) \n" \
                        "6.Topics_reset(reset the monitored topics) \n" \
                        "7.exit")
        if command == 1:
