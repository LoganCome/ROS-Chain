...
class OdometryParser(BaseParser):
    ...
    def parse(self,msg,topic):
        ...
        message_dict = dict(zip(['time','relation_x','relation_y','relation_linear_x','relation_linear_y','encryption_method','topic'],
                                [str(format_time),str(self._msg.pose.pose.position.x),str(self._msg.pose.pose.position.y),str(self._msg.twist.twist.linear.x),str(self._msg.twist.twist.linear.y),str(self._enc_method),'Odometry']))
