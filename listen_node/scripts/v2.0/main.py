...
import scripts.v4.circle_queue
import threading
# -*- coding = utf-8 -*-
...
def listen():
    #sm4密钥读取
    fileread_threads[0] = threading.Thread(target=read_txt,args=('./deploy_args/sm4_key.txt',pyargs.lines,0,))
    #节点名称读取
    fileread_threads[1] = threading.Thread(target=read_txt,args=('./deploy_args/node_names.txt',pyargs.lines,1,))
    #监听端口读取
    fileread_threads[2] = threading.Thread(target=read_txt,args=('./deploy_args/ports.txt',pyargs.lines,2,))
    #topic话题名称读取
    fileread_threads[3] = threading.Thread(target=read_txt,args=('./deploy_args/topic_names.txt',pyargs.lines,3,))
    #sm2公钥读取（当前版本尚未使用）
    fileread_threads[4] = threading.Thread(target=read_txt,args=('./deploy_args/sm2_pubkey.txt',pyargs.lines,4,))
    ...
    for i in range(pyargs.lines[1].__len__()):
        ...
        pyargs.plain_queue.append(scripts.v4.circle_queue.queue(10))
        print isinstance(pyargs.plain_queue[i], scripts.v4.circle_queue.queue)
