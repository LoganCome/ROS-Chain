...
from Send_Control import read_txt, Message_Send,Message_Send_Const
from Robot import robot_plain_topic_callback_test
from sm2 import SM2
fileread_threads = [None]*4
monitor_threads = []
...
if __name__ == '__main__':
    ...
    for i in range(lines[0].__len__()):
        monitor_threads.append(threading.Thread(target=robot_plain_topic_callback_test,args=(plain_queue[i],)))
    for i in range(lines[0].__len__()):
    ...
    while Continue:
        ...
        if crypt_method == 1:
            ...
            if bulk_or_no == 1:
                ...
                for bot_num in range(lines[0].__len__()):
                    # print lines[2][bot_num]
                    Message_Send(sm4_queue[bot_num], Robot.Master_URI, int(lines[2][bot_num]),1,lines[0][bot_num],bot_num+1)
            elif bulk_or_no == 0:
                bot_num = list(input('Please enter the num of the bot(1,2,3...10) u want to encrypt in sm4:\n'))
                len_bot_num = bot_num.__len__()
                len_bots = lines[0].__len__()
                bot_plain = []
                for i in range(len_bot_num):
                    Message_Send(sm4_queue[bot_num[i]-1], Robot.Master_URI, int(lines[2][bot_num[i]-1]),1,lines[0][bot_num[i]-1],bot_num[i])
                ...
                for i in range(bot_plain.__len__()):
                    Message_Send(plain_queue[bot_plain[i]-1], Robot.Master_URI, int(lines[2][bot_plain[i]-1]),3,None,bot_plain[i])
            else:
                print '----------------Warning!----------------\n'
                ...
                if cons_bulk_or_no == 3:
                    ...
                    for i in range(len_bot_num):
                        ...
                        message_send_threads.append(threading.Thread(target=Message_Send_Const,args=(sm4_queue[bot_num[i]-1], Robot.Master_URI, int(lines[2][bot_num[i]-1]),1,lines[0][bot_num[i]-1],bot_num[i],)))
                    for i in range(len_bots):
                        bot_plain.append(i+1)
                    for i in range(len_bot_num):
                        if bot_num[i] in bot_plain:
                    ...
                    for i in range(bot_plain.__len__()):
                        ...
                        message_send_threads.append(threading.Thread(target=Message_Send_Const,args=(plain_queue[bot_plain[i]-1], Robot.Master_URI, int(lines[2][bot_plain[i]-1]),3,None,bot_plain[i],)))
        ...
        if crypt_method == 2:
            ...
            if bulk_or_no == 1:
                ...
                for bot_num in range(lines[0].__len__()):
                    ...
                    # print lines[2][bot_num]
                    Message_Send(plain_queue[bot_num], Robot.Master_URI, int(lines[2][bot_num]),2,key,bot_num+1)
            elif bulk_or_no == 0:
                ...
                for i in range(len_bot_num):
                    key = list(sm2.key_produce())
                    Message_Send(plain_queue[bot_num[i]-1], Robot.Master_URI, int(lines[2][bot_num[i]-1]),2,key,bot_num[i])
                for i in range(len_bots):
                    bot_plain.append(i+1)
                for i in range(len_bot_num):
                    if bot_num[i] in bot_plain:
                ...
                if cons_bulk_or_no == 3:
                    ...
                    while True:
                        for bot_num in range(lines[0].__len__()):
                            # print lines[2][bot_num]
                            Message_Send(plain_queue[bot_num], Robot.Master_URI, int(lines[2][bot_num]),2,keys[bot_num],bot_num+1)
                        time.sleep(0.5)
                else:
                    bot_num = list(input('Please enter the num of the bot(1,2,3...10) u want to encrypt in sm2:\n'))
                    len_bot_num = bot_num.__len__()
                    ...
                    for i in range(len_bot_num):
                        # Message_Send(sm4_queue[bot_num[i]-1], Robot.Master_URI, int(lines[2][bot_num[i]-1]),1)
                        message_send_threads.append(threading.Thread(target=Message_Send_Const,args=(plain_queue[bot_num[i]-1], Robot.Master_URI, int(lines[2][bot_num[i]-1]),2,keys[i],bot_num[i],)))
                    for i in range(len_bots):
                        bot_plain.append(i+1)
                    for i in range(len_bot_num):
                        if bot_num[i] in bot_plain:
        ...
        if crypt_method == 3:
            ...
            if bulk_or_no == 1:
                ...
                for bot_num in range(lines[0].__len__()):
                    # print lines[2][bot_num]
                    Message_Send(plain_queue[bot_num], Robot.Master_URI, int(lines[2][bot_num]),3,None,bot_num+1)
            else:
                print '----------------Warning!----------------\n'
