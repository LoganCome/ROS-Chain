...
def Recv_data_from_Robot_1():
    ...
    '''绑定端口'''
    Udp_Socket.bind(("192.168.1.107", Robot_1_port))
    '''线程执行任务'''
    while thread_on:
        '''接受端口发送的数据'''
...
def Recv_data_from_Robot_2():
    Udp_Socket = socket(AF_INET, SOCK_DGRAM)
    Udp_Socket.bind(("192.168.1.107", Robot_2_port))
    while 1:
        Recv_Data = Udp_Socket.recvfrom(1024)
        Recv_msg = Recv_Data[0].decode('utf-8')
        Recv_msg = Recv_msg.split("+")
...
def data_process(x, y, name):
    ...
    r.rpushx(name + "_x", 0)
    r.rpushx(name + "_y", 0)
    temp_x = r.brpop(name + "_x")
    temp_y = r.brpop(name + "_y")
    '''比较此时拿到的数据和list中上一个数据新型比对，若相同则不入库，否则append到末尾'''
    if x == temp_x and y == temp_y:
        r.rpushx(name + "_x", temp_x)
        r.rpushx(name + "_y", temp_y)
    else:
        r.rpushx(name + "_x", temp_x)
        r.rpushx(name + "_y", temp_y)
        r.rpushx(name + "_x", x)
